(function () {
  var config = JSON.parse('{"defaultAvatar":"./img/default-avatar.jpg","rtl":false,"headless":false,"commLang":"ID"}');
  var commentTranslations = JSON.parse('{"defaultName":"Tamu","defaultDate":"at that time"}');
...
$('document').ready(function () {
  ...
  $('#voteBtnSubmt').click(function () {
    var stdRInput = $('input[type="radio"]');
    var checkedRInput = $('input[type="radio"]:checked');
    var voteCountValue = $('#voteCountValue').html();

    var value = checkedRInput.val();
    var valueFinVote = parseInt(voteCountValue);
    var resRInput = parseInt(value);
...
window.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('a, .to-link, .comment__img, .search-button').forEach((el) => {
    el.addEventListener('click', function (e) {
      e.preventDefault();

      const targetElement = document.querySelector('#myForm');
